...
class UnofficialChat extends React.Component { 
    ...
    refreshChat = setInterval(function() {
        ...
        // axios.post('http://localhost:8000/api/chat/unofficial/refresh', JSON.stringify(objectToSend))
    ...
    componentDidMount() {
        ...
        axios.get('http://localhost:8000/api/chat/unofficial/lastmessage')
        .then(res => {
            console.log(res);
    ...
    onSend = e => {
        ...
        axios.post('http://localhost:8000/api/chat/unofficial/send', JSON.stringify(messageToSend))
        .then(res => {
            let now = new Date();
